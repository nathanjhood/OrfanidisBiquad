...
AutoComponentLookAndFeel::AutoComponentLookAndFeel()
{
    ...
    //* Button style */
    setColour(juce::Button::buttonDown, juce::Colours::antiquewhite);
    ...
    setColour(juce::TextButton::buttonOnColourId, juce::Colours::antiquewhite);
...
void AutoComponent::resized()
{
    ...
    for (auto* s : sliders)
    {
        ...
        s->slider.setBounds (x - offset, absCentreY - 20, 85, 80);
        x = s->slider.getRight();
        first = false;
    }
    ...
    auto bottomUp = getBoundsInParent().getBottom() - 80;

    for (auto* b : boxes)
    {
        int offset = first ? -40 : -5;
        b->box.setBounds (x - offset, bottomUp, 70, 20);
        x = b->box.getRight();
        first = false;
    }
    ...
    for (auto* b : buttons)
    {
        ...
        b->button.setBounds (x - offset, bottomUp, 70, 20);
        x = b->button.getRight();
        first = false;
    }
}
